...
var base_model = preload("res://Monsters/M3.png")
var DMG_model= preload("res://Monsters/M3 damage.png")
	...
	if Enemy_health.value <= 0 or Player_health.value <=0 or Player_Turn_NUM > Enemy_Turn_NUM:
		...
		if RunChance_times != 0:
			if RunChance > 30:
				...
				get_tree().change_scene("res://Map/Room3/Room3.tscn")
			else:
				RunChance_times -= 1
...
func _process(delta):
	...
	if Enemy_health.value <= 0:
		MusicControl.play_basic_music()
		get_tree().change_scene("res://Map_Clear/2/Room3/Room3.tscn")
