...
var base_model = preload("res://Monsters/boss.png")
var DMG_model= preload("res://Monsters/boss damage.png")
	...
	if Enemy_health.value <= 0 or Player_health.value <=0 or Player_Turn_NUM > Enemy_Turn_NUM:
		...
		if RunChance_times != 0:
			if RunChance > 30:
				...
				get_tree().change_scene("res://Map/Room6/Room6.tscn")
			else:
				RunChance_times -= 1
...
func Enemy_DMG_System():
	...
	OS.delay_msec (150)
	var Enemy_dmg =0
	var One_hit_chance = randi()%51
	if One_hit_chance == 50:
		Enemy_dmg = 100
...
func _process(delta):
	...
	if Enemy_health.value <= 0:
		MusicControl.play_basic_music()
		get_tree().change_scene("res://Map_Clear/4/Room6/Room6.tscn")
